...
ARGS["filename"] = (
    [],
    {"type": str, "help": "the python filename (or directory with [-d] or [--folder]) to add the license file to"},
)
...
ARGS["-l"] = (
    ["--license"],
    {"default": LICENSE_FILE, "help": """(optional) The filename of the license to add to the Python files"""},
)
...
def add_license(header, lic, sep=os.linesep):
    if has_license(header):
        raise Exception(
            "There is already a license in the provided header. Please rem_license prior to adding a new one."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Script for adding a license 
